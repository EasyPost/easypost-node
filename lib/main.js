...
module.exports = function (apiKey, options) {
  ...
  easypost.Resource.retrieve = function(cls, params, cb) {
    var url = easypost.Resource.classUrl(cls) + '/'
    url += (typeof params === 'object') ? params.id : params
    var opts = {}
    if (typeof params === 'object')
      opts = params.apiKey
    var requestor = new Requestor(opts);
  ...
  easypost.Shipment.prototype.lowestRate = function(carriers, services) {
    ...
    if (services !== null) {
      ...
      for(var k=0; k < services.length; k++) {
        // excluded services are prefixed with '!'
        if (services[k].substring(0, 1) === '!')
          excludedServices.push(services[k].replace('!', '').toLowerCase())
        else
          services[k] = services[k].toLowerCase();
      }
    }

    for(var z=0; z < this.rates.length; z++) {
      if (carriers !== null && carriers.length > 0 && carriers.indexOf(this.rates[z].carrier.toLowerCase()) === -1) {
        continue;
      }
      if (services !== null && services.length > 0 && services.indexOf(this.rates[z].service.toLowerCase()) === -1) {
        continue;
      }
      if (_.contains(excludedServices, this.rates[z].service.toLowerCase()))
        continue
      if (lowestRate === null || parseFloat(lowestRate.rate) > parseFloat(this.rates[z].rate)) {
        lowestRate = this.rates[z];
      }
    }
