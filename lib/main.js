...
module.exports = function (apiKey, options) {
  ...
  easypost.Shipment.prototype.lowestRate = function(carriers, services) {
    ...
    _.each(carriers, function(carrier, i) {
      carriers[i] = carrier.toLowerCase()
    })
    ...
    _.each(services, function(service, i) {
      ...
      if (service.substring(0, 1) === '!') {
        excludedServices.push(service.replace('!', ''))
        delete services[i]
      } else {
        services[i] = service
      }
    ...
    _.each(this.rates, function(rate, i) {

      if (!_.isEmpty(carriers) && !_.contains(carriers, rate.carrier.toLowerCase()))
        return

      if (!_.isEmpty(services) && !_.contains(services, rate.service.toLowerCase()))
        return

      if (_.contains(excludedServices, rate.service.toLowerCase()))
        return

      if (_.isEmpty(lowestRate) || ( _.has(lowestRate, 'rate') && ( parseFloat(lowestRate.rate) > parseFloat(rate.rate) ) ) )
        lowestRate = rate
