...
export default (easypostClient) =>
  ...
  class PickupService extends baseService(easypostClient) {
     ...
     * @param {Object} pickups An object containing a list of {@link Address pickups} and pagination information.
     * @param {Number} pageSize The number of records to return on each page
     * @returns {EasyPostObject|Promise<never>} The retrieved {@link EasyPostObject}-based class instance, or a `Promise` that rejects with an error.
     */
    static async getNextPage(pickups, pageSize) {
      const url = 'pickups';
      const pickupsArray = pickups.pickups;
      return this._getNextPage(url, pickupsArray, pageSize);
    }
